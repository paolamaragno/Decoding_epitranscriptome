...
## Read2Gene function
read_to_gene <- function(alignments, genes) {
	...
	reads <- names(alignments)
	names(reads) <- names(genes)[subjectHits(hits_unique)]
	reads
}
...
### Alignments data
bam.files.unlabeled <- list.files(paste0(resultsDir,"/",unlabeled_time),recursive=TRUE)
bam.files.unlabeled <- bam.files.unlabeled[grepl("nanoid/bam.RData",bam.files.unlabeled)]

bam.files.fullylabeled <- list.files(paste0(resultsDir,"/",fullylabeled_time),recursive=TRUE)
bam.files.fullylabeled <- bam.files.fullylabeled[grepl("nanoid/bam.RData",bam.files.fullylabeled)]

bam.unlabeled <- lapply(bam.files.unlabeled,function(i){get(load(paste0(resultsDir,"/",unlabeled_time,"/",i)))})
bam.fullylabeled <- lapply(bam.files.fullylabeled,function(i){get(load(paste0(resultsDir,"/",fullylabeled_time,"/",i)))})

read.annotation.unlabeled <- unlist(lapply(bam.unlabeled,function(i)read_to_gene(i,bedFile)))
read.annotation.fullylabeled <- unlist(lapply(bam.fullylabeled,function(i)read_to_gene(i,bedFile)))

read.annotation.unlabeled <- split(read.annotation.unlabeled,names(read.annotation.unlabeled))
read.annotation.fullylabeled <- split(read.annotation.fullylabeled,names(read.annotation.fullylabeled))

commonGenes <- intersect(names(read.annotation.unlabeled),names(read.annotation.fullylabeled))

readsNumber <- apply(cbind(sapply(read.annotation.unlabeled[commonGenes],length)
						  ,sapply(read.annotation.fullylabeled[commonGenes],length)),1,min)

set.seed(1)
final.reads.unlabeled <- unlist(sapply(commonGenes,function(i)sample(read.annotation.unlabeled[[i]],readsNumber[[i]])))
final.reads.fullylabeled <- unlist(sapply(commonGenes,function(i)sample(read.annotation.fullylabeled[[i]],readsNumber[[i]])))
...
read.based.parameters.unlabeled <- read.based.parameters.unlabeled[final.reads.unlabeled,]
...
read.based.parameters.fullylabeled <- read.based.parameters.fullylabeled[final.reads.fullylabeled,]
...
for(seedTmp in seeds)
{
	for(sizeTmp in sizes)
	{
		...
		shuffling_tmp = sample(seq_along(y_train_tmp),length(y_train_tmp))
		y_train_tmp = y_train_tmp[shuffling_tmp]
		x_train_tmp = x_train_tmp[shuffling_tmp,]

		train_reads_tmp = rownames(x_train_tmp)
