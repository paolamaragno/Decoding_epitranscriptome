...
if(file.exists(where=paste0(resultsFolder,"nanoid/raw.signal.five.mers.csv")))
{
  nrowMat <- system(paste0("cat ",paste0(resultsFolder,"nanoid/raw.signal.five.mers.csv")," | wc -l"), intern=TRUE)
...
if(start<=length(index.list))
{
  ...
  for (j in start:length(index.list)){
    ...
    raw.signal.five.mers.tmp <- do.call("rbind", raw.signal.five.mers.tmp)
    
    rownames(raw.signal.five.mers.tmp) <- bamNames[index.list[[j]]]
 
    colnames(raw.signal.five.mers.tmp) = col.names
 
    fwrite(x=raw.signal.five.mers.tmp
           ,file=paste0(resultsFolder,"nanoid/raw.signal.five.mers.csv")
           ,append=TRUE)
