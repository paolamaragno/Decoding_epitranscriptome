...
if(file.exists(where=paste0(resultsFolder,"nanoid/raw.signal.five.mers.add.on.csv")))
{
  nrowMat <- system(paste0("cat ",paste0(resultsFolder,"nanoid/raw.signal.five.mers.add.on.csv")," | wc -l"), intern=TRUE)
...
if(start<=length(index.list))
{
  ...
  for (j in start:length(index.list)){
    ...
    raw.signal.five.mers.add.on.tmp <- do.call("rbind", raw.signal.five.mers.add.on.tmp)
    
    rownames(raw.signal.five.mers.add.on.tmp) <- bamNames[index.list[[j]]]
 
    colnames(raw.signal.five.mers.add.on.tmp) = col.names
 
    fwrite(x=raw.signal.five.mers.add.on.tmp
           ,file=paste0(resultsFolder,"nanoid/raw.signal.five.mers.add.on.csv")
           ,append=TRUE)    
