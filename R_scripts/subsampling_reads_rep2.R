...
subsampling <- function(file_fastq) {
  ...
  write(selected_genes, paste0(path,"/selected_genes_rep2.txt"))
  ...
  system(command=paste('cat', paste0(strsplit(path_gtf_file, '\\.gtf')[[1]][1], '.genes.bed'), '| grep -f', paste0(path,"/selected_genes_rep2.txt"), '>', paste0(strsplit(path_gtf_file, '\\.gtf')[[1]][1], '.selected_genes_rep2.bed')))
